...
function CombineSettings:load()
	...
	self.isDedi = g_currentMission.connectedToDedicatedServer
	...
	if CombineSettings.debug then 
		print("isMP: "..tostring(self.isMP))
		print("isDedi: "..tostring(self.isDedi))
	end
	-- if self.isMP then
	...
	FSBaseMission.onConnectionFinishedLoading = Utils.appendedFunction(FSBaseMission.onConnectionFinishedLoading, CombineSettings.loadSettingsFromServer)
...
function CombineSettings:loadSettingsFromServer()
    if CombineSettings.debug then print("CombineSettings:loadSettingsFromServer") end
	g_client:getServerConnection():sendEvent(xpCombineEvent.new(g_combinexp.powerBoost, g_combinexp.powerDependantSpeed.isActive, g_combinexp.timeDependantSpeed.isActive))
end
...
function CombineSettings:onSettingsStateChanged(state, element, isChangedUp)
	...
	if element.id == "combineGameplay" then
		...
		-- g_client:getServerConnection():sendEvent(xpCombineEvent.new(xpCombine, state))
		if state == 1 then
